...
def entropy(y):
    ...
    for i in range(len(e)):
        sum = np.sum([(-c[i]/total) * np.log2(c[i]/total)])
        hy = hy + sum
...
def mutual_information(x, y):
    ...
    for j in partitionOfX:
        sum = np.sum((len(partitionOfX[j])/total) * entropy(y[partitionOfX[j]]))
        hy_x = hy_x + sum
...
def id3(x, y, attribute_value_pairs=None, depth=0, max_depth=5):
    ...
    * Leaf nodes are (majority) class labels
    ...
    for i, val in(attribute_value):
        ...
        infoGain[(i,val)] = mutual_information(bin, y)
...
#            split_x.append(np.array(x)[k,:]) 
    ...
    for split_on, index in partition_new.items():
        ...
        tree[(attribute, value, decide_upon)] = id3(split_x, split_y, attribute_value_pairs=attribute_value, max_depth=max_depth, depth=depth)
